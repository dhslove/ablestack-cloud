...
if [ "$1" == "2" ] ; then
    /usr/bin/systemctl is-active --quiet mold && touch "${management_upgrade_state}.was-active" || true
    /usr/bin/systemctl is-enabled --quiet mold && touch "${management_upgrade_state}.was-enabled" || true
fi
...
if [ -f "${management_upgrade_state}.was-active" ] ; then
    echo "Restarting Mold management server to apply packaged database upgrades"
    if ! /usr/bin/systemctl start mold ; then
        /usr/bin/systemctl --no-pager status mold || true
        exit 1
    fi
    sleep 10
