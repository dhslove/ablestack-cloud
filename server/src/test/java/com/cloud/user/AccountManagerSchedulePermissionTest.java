...
public class AccountManagerSchedulePermissionTest {
    ...
    public void checksUserAndKeyPermissionsWithoutChargingRateLimitAgain() {
        ...
        Mockito.doThrow(new PermissionDeniedException("project or key denies VM schedule")).when(acl)
                .checkAccess(user, "createVMSchedule", permission);
        try {
            ...
            Mockito.verify(acl).checkAccess(user, "createVMSchedule", permission);
            Mockito.verify(rate, Mockito.never()).checkAccess(Mockito.any(User.class), Mockito.anyString(), Mockito.<ApiKeyPairPermission[]>any());
            Mockito.verify(rate, Mockito.never()).checkAccess(Mockito.any(Account.class), Mockito.anyString(), Mockito.<ApiKeyPairPermission[]>any());
        }
    }
