...
const isFtctlManagedKvmVm = (record, store) => {
  return ['Admin'].includes(store.userInfo.roletype) &&
    record.hypervisor === 'KVM' &&
    record.vmtype !== 'sharedfsvm'
}

const isFtctlUnsafeVmState = (record) => {
  return ['Destroyed', 'Expunging', 'Error'].includes(record.state) || record.hostcontrolstate === 'Offline'
}

const getFtctlField = (record, ...keys) => {
  for (const key of keys) {
    if (record && record[key] !== undefined && record[key] !== null && record[key] !== '') {
      return record[key]
    }
    if (record && record.details && record.details[key] !== undefined && record.details[key] !== null && record.details[key] !== '') {
  ...
  children: [
    {
      ...
      actions: [
        ...
        {
          api: 'registerFtctlProtection',
          icon: 'setting-outlined',
          label: 'FTCTL Protection',
          dataView: true,
          popup: true,
          show: (record, store) => {
            return 'registerFtctlProtection' in store.apis && isFtctlManagedKvmVm(record, store)
          },
          disabled: (record) => { return isFtctlUnsafeVmState(record) },
          component: shallowRef(defineAsyncComponent(() => import('@/views/compute/RegisterFtctlProtection.vue')))
        },
        {
          ...
          label: 'FTCTL Pause',
          dataView: true,
          message: 'Pause FTCTL protection for this virtual machine?',
          show: (record, store) => {
            return 'pauseFtctlProtection' in store.apis && isFtctlManagedKvmVm(record, store)
          },
          disabled: (record) => {
            const adminState = getFtctlField(record, 'ftctladminstate', 'adminstate', 'ftctl.last.admin.state')
            return isFtctlUnsafeVmState(record) || adminState === 'paused'
          },
        ...
        },
        {
          ...
          label: 'FTCTL Resume',
          dataView: true,
          message: 'Resume FTCTL protection for this virtual machine?',
          show: (record, store) => {
            return 'resumeFtctlProtection' in store.apis && isFtctlManagedKvmVm(record, store)
          },
          disabled: (record) => {
            const adminState = getFtctlField(record, 'ftctladminstate', 'adminstate', 'ftctl.last.admin.state')
            return isFtctlUnsafeVmState(record) || (adminState !== null && adminState !== 'paused')
          },
          args: ['virtualmachineid'],
          mapping: {
            virtualmachineid: {
        ...
        },
        {
          ...
          icon: 'swap-outlined',
          label: 'FTCTL Failover',
          dataView: true,
          message: 'Trigger FTCTL failover for this virtual machine?',
          show: (record, store) => {
            return 'failoverFtctlProtection' in store.apis && isFtctlManagedKvmVm(record, store)
          },
          disabled: (record) => {
            const activeSide = getFtctlField(record, 'ftctlactiveside', 'activeside', 'ftctl.last.active.side')
            return isFtctlUnsafeVmState(record) || activeSide === 'secondary'
          },
          args: ['virtualmachineid'],
          mapping: {
            virtualmachineid: {
        ...
        },
        {
          ...
          label: 'FTCTL Failback',
          dataView: true,
          message: 'Trigger FTCTL failback for this virtual machine?',
          show: (record, store) => {
            return 'failbackFtctlProtection' in store.apis && isFtctlManagedKvmVm(record, store)
          },
          disabled: (record) => {
            const activeSide = getFtctlField(record, 'ftctlactiveside', 'activeside', 'ftctl.last.active.side')
            return isFtctlUnsafeVmState(record) || (activeSide !== null && activeSide !== 'secondary')
          },
          args: ['virtualmachineid'],
          mapping: {
            virtualmachineid: {
        ...
        },
        {
          ...
          label: 'FTCTL Confirm Fence',
          dataView: true,
          message: 'Confirm FTCTL fence for this virtual machine?',
          show: (record, store) => {
            return 'confirmFtctlFence' in store.apis && isFtctlManagedKvmVm(record, store)
          },
          disabled: (record) => {
            const fencingState = String(getFtctlField(record, 'ftctlfencingstate', 'fencingstate', 'ftctl.last.fencing.state') || '').toLowerCase()
            return isFtctlUnsafeVmState(record) || (fencingState !== '' && !['required', 'failed', 'manual-required'].includes(fencingState))
          },
          args: ['virtualmachineid'],
          mapping: {
            virtualmachineid: {
        ...
        },
        {
          api: 'clearFtctlFence',
          icon: 'stop-outlined',
          label: 'FTCTL Clear Fence',
          dataView: true,
          message: 'Clear FTCTL fence state for this virtual machine?',
          show: (record, store) => {
            return 'clearFtctlFence' in store.apis && isFtctlManagedKvmVm(record, store)
          },
          disabled: (record) => {
            const fencingState = String(getFtctlField(record, 'ftctlfencingstate', 'fencingstate', 'ftctl.last.fencing.state') || '').toLowerCase()
            return isFtctlUnsafeVmState(record) || fencingState === 'clear'
          },
          args: ['virtualmachineid'],
          mapping: {
            virtualmachineid: {
